...
export class LoginComponent implements OnInit, OnDestroy {
  ...
  ngOnDestroy() {
    ...
    this.unsubscribe.complete(); // Появляется и завершается поток, чтобы сделать отписку
  }
  ...
  onLogin() {
    ...
    this.authService
      ...
      // Из потока берет данные пока не появится поток unsubscribe, а он появится когда компонент начнет разрушаться
